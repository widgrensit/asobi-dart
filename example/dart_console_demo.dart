// End-to-end console demo for asobi-dart against widgrensit/sdk_demo_backend.
//
...
//
// Then run:
//   dart run example/dart_console_demo.dart
//
// Or point at a different host:
//   ASOBI_URL=http://my.host:8084 dart run example/dart_console_demo.dart
...
Future<void> main() async {
  ...
  // server pairing us immediately. `match.matched` has its own typed stream;
  // onMatchEvent only carries a Lua script's game.broadcast(...) events.
...
Future<_Player> _spawnPlayer(String label, Map<String, dynamic> url) async {
  ...
  final rng = Random();
  final username =
      'demo_${label}_${DateTime.now().millisecondsSinceEpoch}_${rng.nextInt(10000)}';
